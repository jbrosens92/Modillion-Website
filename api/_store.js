...
/* ============================================================
   ...
   has. Six of them come back in one pipelined request, so a tab
   polling every few seconds costs one small call and no reads of
   ...
   That is the only property asked of it.
   ============================================================ */
export async function readStamps(sets) {
  const list = (sets || []).slice();
  const out = {};
  if (!list.length) return out;
  const res = await redisPipeline(list.map(s => ["GET", KEY("stamp", s)]));
  list.forEach((s, i) => { out[s] = Number(res[i]) || 0; });
  return out;
}
