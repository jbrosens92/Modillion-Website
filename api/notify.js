/* ============================================================
   ...
   THE ALLOW LIST IS THE THING WORTH HAVING EXACTLY ONCE. This
   file used also to export sendDigest() for the weekly news blast,
   so that the blast never held a Resend key of its own and every
   address it used still went through allowedRecipient(). The blast
   was removed on 2026-09-15 — it had never run — and those exports
   went with it. If a second kind of mail is ever added, add it
   here for the same reason rather than giving it its own endpoint.
   ============================================================ */
...
/* The one call to the mail service. Everything that sends ends up here,
   so there is one place where the firm's return address is spent. */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const mail = clip(task.title, 300).trim()
